...
对于 Windows 用户，我们建议直接使用系统自带的 **远程桌面连接** 而不需要安装额外的第三方软件。由于租赁后没有开启 **SSH**，所以不推荐第一次使用SSH连接

**使用远程桌面连接**

1. 按 ``Win + R``，输入 ``mstsc`` 打开远程桌面连接。
2. 输入服务器 IP 地址:端口号，点击 **连接**。
3. 输入用户名和密码即可登录。

**使用 CMD/PowerShell 进行 SSH 连接（不推荐）**

Windows 10 及以上版本自带了 OpenSSH 客户端,但是第一次使用无法连接。
